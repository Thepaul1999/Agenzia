...
// Funzione helper per le query di count
async function getCount(supabaseQuery: any): Promise<number> {
...
<div className="stats-kpi-grid">
  {[
    { n: todayCount as number, l: 'Oggi', accent: true },
    { n: weekCount as number, l: 'Ultimi 7 giorni' },
    { n: monthCount as number, l: 'Ultimi 30 giorni' },
    { n: totalSiteVisits as number, l: 'Totale visite' },
  ].map((k: { n: number; l: string; accent?: boolean }) => (
...
<div className="stats-kpi-grid" style={{ marginBottom: '2.5rem' }}>
  {[
    { n: list.length as number, l: 'Totale immobili' },
    { n: list.filter((i) => i.pubblicato).length as number, l: 'Pubblicati' },
    { n: list.filter((i) => i.featured).length as number, l: 'In evidenza' },
    { n: totalPropertyViews as number, l: 'Visite totali' },
  ].map((k: { n: number; l: string }) => (
    <div key={k.l} className="stats-kpi">
